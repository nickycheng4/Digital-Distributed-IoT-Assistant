...
from .mappings import command_device, parameter_device

def remove_words( s ):

    stopwords = ['please ','a ','the ']
    for sw in stopwords:
        s = s.replace( sw, '' )
...
def cmd_extract(doc, nlp, embeddings):
    ...
    for cmdDoc in cmdDocs:
        ...
        verb_adps = [
            t 
            for t in cmdDoc 
            if t.dep_ in ["prep", "prt"] and t.head.pos_ == "VERB" and t._.is_condition == False and t.text not in not_para_adps
        ]

        verbs = [
            t 
            for t in cmdDoc 
            if t.pos_ == "VERB" and t._.is_condition == False 
        ]


        if len(non_conj_dev_nouns) > 1:

            not_dev = [ dev for dev in non_conj_dev_nouns if remove_words(str(dev)) not in parameter_device ]
            non_conj_dev_nouns = list( set(non_conj_dev_nouns) - set(not_dev) ) 

            for ndev in not_dev:
                if ndev in parameter_device[non_conj_dev_nouns[0]]:
                    if non_conj_parameter_nouns == []:
                        non_conj_parameter_nouns.append(ndev)
                    else:
                        conj_parameter_nouns.append((non_conj_parameter_nouns[0],ndev))
    ...
    for parameter in parameters:
        ...
        command = [remove_words(str(p)) for p in parameter]
